...
async function convert({
  ...
  try {
    ...
    log.info(`Rendering ${totalFrames} frames @ ${fps}fps, crf=${crf}, preset=${preset} → ${absOut}`);
    ...
    log.info(`Done → ${absOut} (${(stat.size / 1024 / 1024).toFixed(2)} MB, ${totalFrames} frames, ${duration}s @ ${fps}fps)`);
  } catch (err) {
    if (encoder) { try { encoder.kill(); } catch {} }
    throw err;
  } finally {
    if (capture) { try { await capture.close(); } catch {} }
