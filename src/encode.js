...
function openEncoder({ outPath, fps, crf = 15, preset = 'slow', scaleTo = null, verbose = false }) {
    ...
    write: (pngBuffer) =>
      new Promise((resolve, reject) => {
        if (exitInfo)
          return reject(
            new Error(
              `ffmpeg died early (code=${exitInfo.code} signal=${exitInfo.signal})\n${stderrBuf.slice(-2000)}`,
            ),
          );
      ...
      new Promise((resolve, reject) => {
        if (exitInfo) {
          if (exitInfo.code === 0) resolve();
            ...
            reject(
              new Error(
                `ffmpeg exited code=${exitInfo.code} signal=${exitInfo.signal}\n${stderrBuf.slice(-2000)}`,
              ),
            );
          return;
        }
        ff.once('close', (code, signal) => {
          ...
          if (code === 0) resolve();
            ...
            reject(
              new Error(`ffmpeg exited code=${code} signal=${signal}\n${stderrBuf.slice(-2000)}`),
            );
        });
        ff.stdin.end();
      }),
